...
const DoublePana = () => {
    ...
    const [selectedRow, setSelectedRow] = useState(0); // Start with row 0
    const [gameType, setGameType] = useState('CLOSE'); // Default game type
    const navigate = useNavigate();
    const { marketName } = useParams();
    const { state } = useLocation();
    const marketId = state?.marketId || 1; // Retrieve market ID from navigation state or default
    ...
    const getMarketAndGameType = () => {
        ...
        return { table: 'bids', marketIdField: 'market', gameTypeId: 13 }; // Default for other markets
    };
