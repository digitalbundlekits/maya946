...
const SingleBulkDigits = () => {
    ...
    const [pointValue, setPointValue] = useState(); // Default point value
    const [showReviewModal, setShowReviewModal] = useState(false);
    const [reviewData, setReviewData] = useState({});
    const navigate = useNavigate();
    const { marketName } = useParams();
    const { state } = useLocation();
    const marketId = state?.marketId || 1; // Correctly using marketId from state
    ...
    const getMarketAndGameType = () => {
        ...
        return { table: 'bids', marketIdField: 'market', marketId, gameTypeId: 10 }; // Default for other markets
    };
    ...
    const confirmSubmit = async () => {
        try {
            ...
            const response = await axios.post('https://bhoom.miramatka.com/api/placeBet.php', {
                ...
                market_id: marketId,
                bids: reviewData.bids.map(bid => ({ digit: bid.digit, points: bid.points, type: bid.type })),
                totalAmount,
