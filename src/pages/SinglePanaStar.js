...
const SinglePana = () => {
    ...
    const getMarketAndGameType = () => {
        ...
        return { table: 'bids', marketIdField: 'market', gameTypeId: 11 };
    };
        ...
        <div className="single-pana-container">
            ...
            <div className="row-selector">
                {Object.keys(panaNumbers).map(num => (
                    <button
                        key={num}
                        className={`row-button ${selectedRow === parseInt(num) ? 'active' : ''}`}
                        onClick={() => handleRowSelect(parseInt(num))}
                    >
                        {num}
                    </button>
                ))}
                ...
                {panaNumbers[selectedRow]?.map((number, index) => (
                    <div key={index} className="pana-input">
                        ...
                        <input
                            ...
                            value={amounts[selectedRow]?.[index] || ''}
